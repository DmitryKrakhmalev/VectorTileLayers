...
{
    /// <summary>
    /// Class for loading OpenMapTiles Json style files
    /// </summary>
    public static class OMTStyleFileLoader
    {
        ...
        public static OMTStyleFile Load(Stream input)
        {
            ...
            var mglStyleFile = new OMTStyleFile(jsonStyle.Name, jsonStyle.Version)
            {
        ...
        private static void ExtractSprites(JsonStyleFile jsonStyle, OMTSpriteAtlas spriteAtlas)
        {
        ...
        private static List<ILayer> ExtractLayers(JsonStyleFile jsonStyle, Dictionary<string, JsonSource> sources, OMTSpriteAtlas spriteAtlas)
        {
            ...
            {
                if (!string.IsNullOrWhiteSpace(vectorTileStyle.Source) && !usedSources.Contains(vectorTileStyle.Source))
        ...
        private static ILayer CreateBackgroundTileLayer(IEnumerable<JsonStyleLayer> styleLayers, OMTSpriteAtlas spriteAtlas)
        {
            ...
            foreach (var styleLayer in styleLayers)
            {
                ...
                if (styleLayer.Type.ToLower().Equals(name))
                {
                    ...
                    var backgroundTileLayer = new OMTBackgroundLayer(paint[0]);
        ...
        private static TileLayer CreateRasterTileLayer(JsonStyleFile jsonStyle, string name, JsonSource source, OMTSpriteAtlas spriteAtlas)
        {
            ...
            if (rasterStyleLayer != null)
            {
                ...
                ((OMTPaint)rasterStyleLayer.Paints.First()).SetFixColor(SKColors.White);
            }
        ...
        private static ILayer CreateVectorTileLayer(JsonStyleFile jsonStyle, string name, JsonSource source, OMTSpriteAtlas spriteAtlas)
        {
